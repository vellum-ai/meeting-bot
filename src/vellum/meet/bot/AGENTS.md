...
## Superseded: this tree is no longer the `vellum` path (2026-07)

Anonymous browser joins do not work and cannot be made to work here. Meet hard-denies our client at knock submission ("You can't join this video call"); Google's sanctioned alternatives are Developer-Preview- or allowlist-gated, and Zoom now requires a token tied to a participant already in the meeting. These are platform-policy limits, not bugs.

The `vellum` provider is therefore moving to the platform's Velay ingress rather than driving its own browser — see [`src/vellum/velay/`](../../velay/gateway-manifest.ts).

**This tree still works and stays for now, but should not be extended.** New `vellum` work belongs in `src/vellum/velay/`.
