 ...
 *
 *   - URL shape (includes `/meet-internal?meetingId=<meetingId>`).
...
describe("DaemonClient", () => {
  test("POSTs buffered events to the ingress URL with bearer auth", async () => {
    ...
    expect(call.url).toBe(
      "http://daemon.local:1234/meet-internal?meetingId=m-abc",
    );
  ...
  test("URL encodes the meeting id", async () => {
    ...
    expect(mock.calls[0]!.url).toBe(
      "http://daemon.local/meet-internal?meetingId=m%2Fweird%20id",
    );
  });
  ...
  test("trims trailing slashes from the daemon URL", async () => {
    ...
    expect(mock.calls[0]!.url).toBe(
      "http://daemon.local:9000/meet-internal?meetingId=m-1",
    );
  });
});
